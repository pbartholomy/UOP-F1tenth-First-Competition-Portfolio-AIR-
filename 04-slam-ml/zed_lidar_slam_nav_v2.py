...
# v2 fix: all caps are now strictly below AUTO_DRIVE_DUTY so speed zones actually slow the car
MODERATE_DUTY_CAP  = 0.063   # was 0.076 — meaningful ~7% reduction at 1.4–2.0 m
SLOW_DUTY_CAP      = 0.060   # was 0.068 — meaningful ~12% reduction at 1.05–1.4 m
CRAWL_DUTY_CAP     = 0.057   # was 0.062 — very slow at 0.7–1.05 m
MIN_EFFECTIVE_DUTY = 0.056   # was 0.060 — floor below which motor stalls
START_BOOST_DUTY   = 0.074   # was 0.078 — gentler launch boost
START_BOOST_LOOPS  = 10      # was 14
REVERSE_DUTY       = 0.08    # was 0.09 — gentler reverse
LOOP_HZ            = 50
DUTY_RAMP_STEP     = 0.004   # was 0.007 — smoother acceleration / deceleration
CURVE_SLOW_STEER   = 0.45    # was 0.58 — begin slowing earlier in curves
CURVE_MIN_SCALE    = 0.50    # was 0.58 — more reduction on sharp steer
...
# ── STEERING PID ─────────────────────────────────────────────
STEER_KP = 3.0    # was 4.0 — less aggressive, reduces oscillation / wall-slap
STEER_KI = 0.012  # was 0.015
STEER_KD = 0.10   # was 0.07 — more damping to prevent overshoot
...
LIDAR_ESTOP_MM      = 400    # was 340 — ESTOP earlier
LIDAR_ESTOP_CLEAR_MM = 620   # was 500 — need more clear space before resuming
BRAKE_LOOPS      = 8         # was 5 — longer brake phase after ESTOP
REVERSE_LOOPS    = 6         # was 8 — shorter reverse
SCAN_LOOPS       = 4
...
SIDE_PANIC_MM          = 580   # was 500 — panic farther from side walls
SIDE_WARN_MM           = 460   # was 420
FRONT_CORNER_WARN_MM   = 850   # was 720 — corner warn farther ahead
WALL_TARGET_MM         = 750   # was 650 — keep more lateral clearance

# ── ZED CAMERA ───────────────────────────────────────────────
CAM_ESTOP_M      = 0.32   # camera nearest-depth ESTOP threshold (m)
ZED_ENABLED      = True
...
SLAM_TRUST_CELLS = 400           # was 600 — activate map steering sooner
...
MAP_STEER_WEIGHT  = 0.32                       # was 0.28 — trust the map more
MAP_CONF_CELLS    = 3000

# Corner prediction
CORNER_DETECT_M    = 2.5   # was 2.0 — detect corners farther ahead
CORNER_FWD_THRESH  = 0.68  # was 0.75 — more sensitive: triggers at 68% of detect range
CORNER_SIDE_RATIO  = 1.3   # was 1.5 — detect corners with less asymmetry required
CORNER_BRAKE       = 0.68  # was 0.76 — more pre-corner speed reduction
CORNER_PRE_STEER   = 0.28  # was 0.22 — more pre-steering into corner
...
def find_best_gap_direction(distances):
    ...
    for i in range(lo, hi + 1):
        ...
        # v2: stronger center-preference penalty (0.20 vs 0.12) reduces hard side-steers
        score         = avg_clearance * (1.0 - 0.20 * offset_norm)
        if score > best_score:
            best_score = score
            best_idx   = i
...
def lidar_panic_override(clearances, front_dist):
    ...
    if front_dist is not None and front_dist < FRONT_CORNER_WARN_MM:
        # v2: slightly reduced corner-warn magnitudes to avoid overcorrection
        mag = 0.76 if front_dist < LIDAR_CRAWL_MM else 0.64
...
def fuse_track_steer(lidar_gap, lidar_corridor, camera_data, clearances, front_dist, slam=None):
    ...
    # v2: more weight on corridor centering and wall following, less on raw gap
    lidar_base = 0.38 * lidar_gap + 0.40 * lidar_corridor + 0.22 * lidar_wall
...
def main():
    ...
    # v2: longer post-ESTOP steer damping for smoother recovery
    estop_exit_dampen   = 0
    ESTOP_EXIT_DAMP_LOOPS = 40   # was 30
    ESTOP_EXIT_DAMP_CAP   = 0.65 # was 0.75
