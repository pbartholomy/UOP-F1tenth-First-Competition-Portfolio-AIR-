#!/usr/bin/env python3
"""
zed_obstacle_node.py — ZED 2i gap/corridor display, obstacle distances, and recording.

Uses pyzed directly (no zed_wrapper needed).

Display (drawn on the center horizontal band of the camera frame):
  - Per-column depth bars across the scan strip (green=open, blue=blocked)
  - Green RECTANGLE outline around the widest open gap
  - Green vertical line at the gap center (heading target)
  - Mode label: CORRIDOR (straight) or GAP (corner), based on front clearance

Publishes (FALLBACK only — corridor_node primary source is always LiDAR):
  /zed/obstacle_front   Float32  m
  /zed/obstacle_left    Float32  m
  /zed/obstacle_right   Float32  m

Video with overlay saved to ~/Desktop/v7Video/videoN.mp4 on each run.
"""
...
class ZedGapNode(Node):
    def __init__(self):
        ...
        # avoids burning a video file on manual driving around the pits.
    ...
    def _tick(self):
        ...
        _txt(frame, f"{mode}  gap={gap_pct}%  F:{front_m:.1f}m", (8, y0 - 6))
