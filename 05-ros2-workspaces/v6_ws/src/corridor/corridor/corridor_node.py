...
# ── TUNING ────────────────────────────────────────────────────
DRIVE_DUTY        = 0.100   # cruise speed
MAX_DUTY          = 0.160   # PI controller ceiling (hard cap on duty output)
START_BOOST_DUTY  = 0.115   # kick from standstill
...
STEER_INTEGRAL_GAIN = 0.24  # how strongly accumulated drift is corrected
STEER_INTEGRAL_MAX  = 0.35  # cap to prevent windup
STEER_INTEGRAL_DECAY= 0.97  # per-loop decay (~halves in ~23 loops / 0.46s)

STEER_D_GAIN   = 2.2        # derivative: anticipate wall approach before it's close
...
GAP_SMOOTH     = 0.68       # EMA for gap steer (higher = snappier turn)
CORNER_EXIT_LOOPS = 18      # loops to stay in gap mode after corner clears (~360ms)
...
ERPM_FULL        = 46000   # cruise (replaces DRIVE_DUTY)
ERPM_MODERATE    = 37000   # 85% zone
ERPM_CORNER_PRE  = 33000   # 80% zone (pre-corner slowdown)
ERPM_CORNER_FAST = 34000   # sweeping/wide corner (front dist near CORNER_MM)
ERPM_CORNER_TIGHT= 24000   # hairpin (front dist near ESTOP_MM)
ERPM_SLOW        = 20000   # tight slow zone
ERPM_CRAWL       = 16000   # near-stop crawl
...
def main(args=None):
    ...
    node.get_logger().info("  corridor_node v5 — corridor + corners + ERPM ctrl")
    node.get_logger().info(f"  Cruise: {ERPM_FULL} ERPM  Corner: {ERPM_CORNER_TIGHT}–{ERPM_CORNER_FAST} ERPM (adaptive)")
    ...
    try:
        while rclpy.ok():
            ...
            if front_dist is not None and front_dist <= CORNER_MM:
                ...
                exit_scale = GAP_SCALE * gap_w + STEER_SCALE * corr_w
                servo = max(SERVO_MIN, min(SERVO_MAX,
                            SERVO_CENTER + steer * exit_scale + STEER_TRIM * corr_w))
