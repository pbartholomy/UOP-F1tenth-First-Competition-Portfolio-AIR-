...
# ERPM targets
ERPM_FULL         = 100000   # cruise in corridors
ERPM_MODERATE     =  82000   # front getting close but not corner yet
ERPM_CORNER_FAST  =  65000   # wide / sweeping corner
...
GAP_SCALE         = 0.40   # servo throw during normal corners
...
OBSTACLE_GAP_SCALE   = 0.34  # obstacle avoidance steering — kept moderate to avoid over-steer

# ZED blind-zone corridor centering (COMP: trust-gated, fail-safe-to-straight).
# When LiDAR goes blind (black walls), use ZED left/right to center the car — but
# only when BOTH side readings are plausibly close. A black rubber wall returns no
# stereo depth and the ZED node reports it as "far/unknown"; steering on that would
# drive INTO the wall, so we reject it and just go straight.
ZED_CENTER_GAIN   = 0.30
ZED_CENTER_SMOOTH = 0.10

# COMP robust black-corridor handling
    ...
    try:
        while rclpy.ok():
            ...
            if GAP_FOLLOW_ENABLED and front_dist is not None and front_dist <= effective_corner_mm:
                ...
                # BLIND zone: LiDAR can't see the black walls. Crash-proof default
                # is straighten-out-and-crawl: every blind tick washes the inherited
                # turn angle toward straight, so the car never drives into a wall on
                # a held heading. ZED may add a gentle centering nudge ONLY when both
                # its side readings fall in the trusted close-range window — a black
                # wall with no stereo depth reads "far", and steering on that would
                # drive INTO the wall, so we ignore it and stay straight.
                if steer_dbg == "BLIND":
                    blind_streak += 1
