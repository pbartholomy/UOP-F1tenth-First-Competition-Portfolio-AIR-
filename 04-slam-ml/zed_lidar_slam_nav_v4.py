...
INVERT_MOTOR       = True
MAX_DUTY           = 0.11
AUTO_DRIVE_DUTY    = 0.072
MODERATE_DUTY_CAP  = 0.069
SLOW_DUTY_CAP      = 0.065
CRAWL_DUTY_CAP     = 0.061
MIN_EFFECTIVE_DUTY = 0.058
START_BOOST_DUTY   = 0.082
START_BOOST_LOOPS  = 10
REVERSE_DUTY       = 0.08
LOOP_HZ            = 50
DUTY_RAMP_STEP     = 0.006
CURVE_SLOW_STEER   = 0.50
CURVE_MIN_SCALE    = 0.56
...
IMU_GYRO_MASK     = 0x0004
IMU_HDG_CORRECT_DEG = 15.0
...
LIDAR_FULL_SPEED_MM = 2000
LIDAR_MODERATE_MM   = 1400
LIDAR_SLOW_MM       = 1050
LIDAR_CRAWL_MM      = 700
LIDAR_ESTOP_MM      = 420    # was 390 — trigger sooner; Hokuyo beam passes over low tubes at distance
LIDAR_ESTOP_CLEAR_MM = 495   # was 455
BRAKE_LOOPS      = 2         # was 3
REVERSE_LOOPS    = 3         # was 4
SCAN_LOOPS       = 3         # was 4
HOLD_TIMEOUT_LOOPS = 22      # was 35

# ── FOLLOW-THE-GAP / SAFETY ──────────────────────────────────
CAR_WIDTH_MM           = 240
FTG_DISPARITY_THRESH_MM = 380
FTG_EXTRA_SAMPLES      = 8
FTG_SAFETY_FACTOR      = 2.2
SIDE_PANIC_MM          = 340   # was 300
SIDE_WARN_MM           = 520   # was 460
FRONT_CORNER_WARN_MM   = 1050
WALL_TARGET_MM         = 620   # was 560
...
SLAM_CELL_M      = 0.05
SLAM_MAP_M       = 24.0
SLAM_N           = int(SLAM_MAP_M / SLAM_CELL_M)
SLAM_HIT         = 0.70
SLAM_FREE        = -0.35
SLAM_MAX_LOG     = 4.0
SLAM_MIN_LOG     = -4.0
...
SLAM_TRUST_CELLS      = 700
SLAM_AUTO_RESET_CELLS = 3200
...
LOOKAHEAD_M       = 3.0
LOOKAHEAD_STEP_M  = SLAM_CELL_M * 2
LOOKAHEAD_ANGLES  = list(range(-65, 66, 5))
MAP_STEER_WEIGHT  = 0.12
MAP_CONF_CELLS    = 3000

CORNER_DETECT_M    = 3.0
CORNER_FWD_THRESH  = 0.52
CORNER_SIDE_RATIO  = 1.15
CORNER_BRAKE       = 0.53
CORNER_PRE_STEER   = 0.33
...
NET_ENABLED      = True          # set False to disable ML entirely
NET_MODEL_PATH   = os.path.expanduser("~/Desktop/slam_nav_net.pt")
NET_DATA_DIR     = os.path.expanduser("~/Desktop/slam_nav_data")
NET_LIDAR_BINS   = 120           # downsample 624-step scan to 120 bins for the network
NET_SCALAR_FEATS = 13            # sector(7) + front(1) + cam_steer/conf(2) + floor_steer/conf(2) + slam_norm(1)
NET_MIN_SAMPLES  = 200           # was 400 — train sooner; tight track rarely clears 400 in one run
NET_TRAIN_EPOCHS = 50
NET_BATCH_SIZE   = 64
NET_LR           = 2e-3
NET_BLEND_MAX    = 0.45          # max network weight in final fusion (rule-based is always base)
NET_GOOD_FRONT_MM = 700          # was 1100 — 1100 almost never met on tight track; 700 covers straights + moderate approaches
...
def compute_corridor_centering(clearances):
    ...
    if abs(raw) < 0.09:   # was 0.14 — tighter deadband for earlier centering corrections
        return 0.0
    return raw
...
def compute_wall_following(clearances):
    ...
    right_push = max(0.0, (WALL_TARGET_MM - right_d) / float(WALL_TARGET_MM))
    left_push  = max(0.0, (WALL_TARGET_MM - left_d)  / float(WALL_TARGET_MM))
...
def fuse_track_steer(lidar_gap, lidar_corridor, camera_data, clearances, front_dist, slam=None):
    ...
    lidar_base = 0.13 * lidar_gap + 0.57 * lidar_corridor + 0.30 * lidar_wall  # was 0.15/0.65/0.20
...
class LidarMapDisplay:
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("LiDAR + ZED + SLAM Nav v4 (ML)")
    ...
    def render(self, distances, clearances, steer, zone, front_dist,
               behavior, source, cam, slam_info, net_info):
        s = self._surface
        s.fill((12, 12, 22))
        self._draw_grid(s)
        self._draw_rings(s)
        ...
        self._draw_hud(s, zone, front_dist, behavior, source, steer, clearances, cam, slam_info, net_info)
        pygame.display.flip()
    ...
    def _draw_hud(self, s, zone, front_dist, behavior, source, steer, clearances, cam, slam_info, net_info):
        ...
        net_steer, net_weight, net_samples = net_info
        ...
        lines = [
            ...
            (f"Pose:   x={pose[0]:.1f} y={pose[1]:.1f} th={math.degrees(pose[2]):.0f}°",
                                                        (160, 160, 220)),
            (f"NET:    steer={net_steer:+.2f} w={net_weight:.2f} n={net_samples}",
                                                        (255, 220, 80) if net_weight > 0.01 else (80, 80, 80)),
        ]
        y = 5
...
def maybe_render_map(map_display, lidar, clearances, steer, zone, front,
                     behavior, source, cam, slam_info, net_info, last_map, period):
    now = time.time()
...
class VideoRecorder:
    ...
    def write(self, frame):
        ...
        if self.writer is None:
            ...
            self.path = os.path.join(ZED_RECORD_DIR, f"zed_slam_v4_{ts}.mp4")
            h, w     = frame.shape[:2]
...
# ── NEURAL NETWORK FEATURE HELPERS ───────────────────────────

def downsample_lidar(distances):
    """Compress 624-step LiDAR scan to NET_LIDAR_BINS bins, normalized to [0,1].
    Uses min-per-bin so the closest obstacle in each angular slice is preserved."""
    if not distances:
        return [1.0] * NET_LIDAR_BINS
    arr = np.array(distances, dtype=np.float32)
    arr[arr <= 20] = 5600.0          # treat invalid readings as max range
    arr = np.clip(arr, 0.0, 5600.0) / 5600.0
    n = len(arr)
    bins = []
    for i in range(NET_LIDAR_BINS):
        lo = int(i * n / NET_LIDAR_BINS)
        hi = int((i + 1) * n / NET_LIDAR_BINS)
        hi = max(lo + 1, hi)
        bins.append(float(arr[lo:hi].min()))
    return bins


def build_scalar_features(clearances, camera_data, slam_cells, front_dist):
    """Build the NET_SCALAR_FEATS=13 scalar inputs for the network."""
    cap = float(LIDAR_FULL_SPEED_MM)
    sector_names = ["FAR_LEFT", "LEFT", "CENTER_LEFT", "CENTER",
                    "CENTER_RIGHT", "RIGHT", "FAR_RIGHT"]
    sector_feats = [min(clearances.get(n, cap), cap) / cap for n in sector_names]
    front_norm   = (min(front_dist, cap) / cap) if front_dist is not None else 1.0
    cam_steer    = float(camera_data.get("steer", 0.0))
    cam_conf     = float(camera_data.get("confidence", 0.0))
    floor_steer  = float(camera_data.get("floor_steer", 0.0))
    floor_conf   = float(camera_data.get("floor_conf", 0.0))
    slam_norm    = min(float(slam_cells), float(MAP_CONF_CELLS)) / float(MAP_CONF_CELLS)
    return sector_feats + [front_norm, cam_steer, cam_conf, floor_steer, floor_conf, slam_norm]
...
if TORCH_AVAILABLE:
    class SteeringNet(nn.Module):
        """Lightweight 3-layer MLP: (120 LiDAR bins + 13 scalars) → steering [-1,1].
        ~17K parameters — forward pass is microseconds on CPU at 50 Hz."""
        def __init__(self, n_in=NET_LIDAR_BINS + NET_SCALAR_FEATS):
            super().__init__()
...
class NeuralPolicy:
    """
    Self-supervised steering policy trained on the v4 rule-based system's own output.

    Lifecycle:
      - During a run: add_sample() collects (LiDAR, scalars, rule_steer) in good states.
      - At end of run: save_and_train() saves data to disk, retrains the model.
      - Next startup: the trained model loads automatically and starts blending immediately.

    The network never overrides safety logic — it blends up to NET_BLEND_MAX weight
    into the final steer command, with the rule-based stack always as the base.
    """
    ...
    def __init__(self):
        ...
        self._trained     = False
        self.model        = None
        self._last_net_steer  = 0.0
        self._last_net_weight = 0.0

        if not TORCH_AVAILABLE:
            print("[NET] PyTorch not available — neural policy disabled")
            return

        os.makedirs(NET_DATA_DIR, exist_ok=True)
        self._total_samples = self._count_saved_samples()
        self.model = SteeringNet()

        if os.path.exists(NET_MODEL_PATH):
            try:
                self.model.load_state_dict(
                    torch.load(NET_MODEL_PATH, map_location="cpu", weights_only=True)
                )
                self.model.eval()
                self._trained = True
                print(f"[NET] Model loaded — {self._total_samples} training samples on disk")
            except Exception as e:
                print(f"[NET] WARNING: model file exists but could not load: {e}")
        else:
            if self._total_samples >= NET_MIN_SAMPLES:
                print(f"[NET] {self._total_samples} samples on disk — will train at end of run")
            else:
                print(f"[NET] {self._total_samples}/{NET_MIN_SAMPLES} samples collected so far")

    def _count_saved_samples(self):
        total = 0
        try:
            for f in os.listdir(NET_DATA_DIR):
                if f.endswith(".npz"):
                    try:
                        d = np.load(os.path.join(NET_DATA_DIR, f))
                        total += len(d["steers"])
                    except Exception:
                        pass
        except Exception:
    ...
    def add_sample(self, lidar_bins, scalar_feats, steer_label):
        """Buffer one training sample. Only called in good (non-panic) states."""
        self._buf_lidar.append(lidar_bins)
        self._buf_scalar.append(scalar_feats)
        self._buf_steer.append(float(steer_label))

    def predict(self, lidar_bins, scalar_feats):
        """Return (net_steer, blend_weight). Weight=0 when untrained or not enough data."""
        if not self._trained or self.model is None or not TORCH_AVAILABLE:
            return 0.0, 0.0
        try:
            x = torch.tensor(lidar_bins + scalar_feats, dtype=torch.float32).unsqueeze(0)
            with torch.no_grad():
                net_steer = float(self.model(x).item())
            # Confidence ramps from 0 → 1 as sample count grows from MIN → 5×MIN
            conf   = min(1.0, self._total_samples / float(NET_MIN_SAMPLES * 5))
            weight = NET_BLEND_MAX * conf
            self._last_net_steer  = net_steer
            self._last_net_weight = weight
            return net_steer, weight
        except Exception:
            return 0.0, 0.0

    def get_last(self):
        return self._last_net_steer, self._last_net_weight, self._total_samples + len(self._buf_steer)

    def save_and_train(self):
        """Save this session's data, then retrain from all saved sessions. Blocking."""
        if not TORCH_AVAILABLE or self.model is None:
            return

        # Save buffered samples from this run
        n_new = len(self._buf_steer)
        if n_new >= 50:
            ts   = time.strftime("%Y%m%d_%H%M%S")
            path = os.path.join(NET_DATA_DIR, f"session_{ts}.npz")
            np.savez_compressed(
                path,
                lidar  = np.array(self._buf_lidar,  dtype=np.float32),
                scalar = np.array(self._buf_scalar, dtype=np.float32),
                steers = np.array(self._buf_steer,  dtype=np.float32),
            )
            self._total_samples += n_new
            print(f"\n[NET] Saved {n_new} samples → {path}  (total: {self._total_samples})")
        else:
            print(f"\n[NET] Only {n_new} samples this run — skipping save (need ≥50)")

        if self._total_samples < NET_MIN_SAMPLES:
            print(f"[NET] {self._total_samples}/{NET_MIN_SAMPLES} total samples — need more before training")
            return

        # Load all session files
        all_lidar, all_scalar, all_steer = [], [], []
        for f in sorted(os.listdir(NET_DATA_DIR)):
            if not f.endswith(".npz"):
                continue
            try:
                d = np.load(os.path.join(NET_DATA_DIR, f))
                all_lidar.append(d["lidar"])
                all_scalar.append(d["scalar"])
                all_steer.append(d["steers"])
            except Exception as e:
                print(f"[NET] WARNING: skipping {f}: {e}")

        if not all_lidar:
            return

        X_lidar  = np.concatenate(all_lidar,  axis=0)
        X_scalar = np.concatenate(all_scalar, axis=0)
        y        = np.concatenate(all_steer,  axis=0)

        # Shuffle
        idx = np.random.permutation(len(y))
        X = np.concatenate([X_lidar[idx], X_scalar[idx]], axis=1)
        y = y[idx]
        ...
        loader  = torch.utils.data.DataLoader(dataset, batch_size=NET_BATCH_SIZE, shuffle=True)

        print(f"[NET] Training on {len(y)} samples for {NET_TRAIN_EPOCHS} epochs...")
        ...
        with self._lock:
            ...
            optimizer = optim.Adam(fresh.parameters(), lr=NET_LR, weight_decay=1e-4)
            loss_fn   = nn.HuberLoss(delta=0.25)  # robust to occasional bad labels

            fresh.train()
            for epoch in range(NET_TRAIN_EPOCHS):
                total_loss = 0.0
                for xb, yb in loader:
                    optimizer.zero_grad()
                    pred = fresh(xb)
                    ...
                    loss.backward()
                    nn.utils.clip_grad_norm_(fresh.parameters(), 1.0)
                    optimizer.step()
                    total_loss += float(loss.item())
                if (epoch + 1) % 10 == 0:
                    avg = total_loss / max(1, len(loader))
                    print(f"[NET]   epoch {epoch+1:3d}/{NET_TRAIN_EPOCHS}  loss={avg:.4f}")

            fresh.eval()
            torch.save(fresh.state_dict(), NET_MODEL_PATH)
            # Hot-swap weights into the live model under lock
            self.model.load_state_dict(fresh.state_dict())
            self.model.eval()
            self._trained = True

        print(f"[NET] Training complete — model saved to {NET_MODEL_PATH}")
...
def main():
    ...
    # Neural policy
    neural_policy = NeuralPolicy() if NET_ENABLED else None
    ...
    print()
    print("=" * 72)
    print("  ZED + LiDAR + SLAM NAV  —  v4  (Neural Policy)")
    print("=" * 72)
    ...
    print(f"  NET: {'enabled' if NET_ENABLED and TORCH_AVAILABLE else 'disabled'}  blend_max={NET_BLEND_MAX}  min_samples={NET_MIN_SAMPLES}")
    ...
    _prev_side_panic     = False
    estop_reverse_servo  = SERVO_CENTER
    ...
    estop_exit_dampen     = 0
    ESTOP_EXIT_DAMP_LOOPS = 25   # was 40
    ESTOP_EXIT_DAMP_CAP   = 0.78 # was 0.65

    # Net display state
    _net_steer_disp  = 0.0
    _net_weight_disp = 0.0
    ...
    try:
        while True:
            ...
            # ── NEURAL POLICY BLEND ──────────────────────────────
            # Collect a training sample when the car is in a clean, non-panicking state.
            # Blend the network's prediction into the final steer command proportional
            # to how many training samples have been accumulated.
            if neural_policy is not None and distances:
                lidar_bins   = downsample_lidar(distances)
                scalar_feats = build_scalar_features(
                    clearances, camera_data,
                    slam_mapper.get_observed() if slam_mapper else 0,
                    front_dist
                )

                is_good_state = (
                    not lidar_estop
                    and estop_cooldown <= 0
                    and front_dist is not None
                    and front_dist > NET_GOOD_FRONT_MM
                    and not any(k in steer_source for k in ("LEFT_PANIC", "RIGHT_PANIC", "BOTH_PANIC"))
                    # WARN (CORNER_WARN) is intentionally allowed — corner approach steers
                    # are exactly what the network needs to learn to anticipate
                )
                if is_good_state:
                    neural_policy.add_sample(lidar_bins, scalar_feats, target_steer)

                net_steer, net_weight = neural_policy.predict(lidar_bins, scalar_feats)
                if net_weight > 0.01:
                    # Safety gate: never let the network override a panic or corner situation
                    if not any(k in steer_source for k in ("PANIC", "WARN", "CORNER")):
                        target_steer = (1.0 - net_weight) * target_steer + net_weight * net_steer
                        steer_source += f"+NET({net_weight:.2f})"
                _net_steer_disp  = net_steer
                _net_weight_disp = net_weight

            # Apply EMA to target before PID; panic overrides bypass EMA and PID
            ...
            if lidar_estop:
                if reverse_counter > REVERSE_LOOPS + SCAN_LOOPS:
                    ...
                    send_servo(ser, estop_reverse_servo)
                    reverse_counter -= 1
                    estop_state = "REVERSE"
                ...
                if now - last_print >= print_period:
                    ...
                    net_n  = neural_policy.get_last()[2] if neural_policy else 0
                    print(
                        f"\r[{drive_state}] "
                        ...
                        f"NET:w={_net_weight_disp:.2f}(n={net_n})    ",
                        end=""
                    )
                    last_print = now
            ...
            net_info = neural_policy.get_last() if neural_policy else (0.0, 0.0, 0)

            last_map = maybe_render_map(
                map_display, lidar, clearances, current_steer, zone_label, front_dist,
                BEHAVIOR_NAMES[behavior_mode], steer_source, camera_data,
                slam_info, net_info, last_map, map_period
            )
