#!/usr/bin/env python3
"""
corridor_node.py — v9: corridor centering + gap following.
...
Drive modes:
  0 = AUTONOMOUS  : reactive corridor + gap follow
  1 = MANUAL      : joystick via car_node → /manual_drive
  2 = MAPPING     : same as AUTONOMOUS reactive nav (car drives itself while slam maps)

Joystick (PS4):
  L1 (btn 4 or 9) : kill switch
  R1 (btn 5)      : cycle MANUAL → MAPPING → AUTONOMOUS (managed by mode_manager_node)
"""
...
CORNER_MM      =  700   # enter gap-follow mode when front is this close
ESTOP_MM       =  300   # hard stop

# ERPM targets
ERPM_FULL         =  95000   # cruise in corridors
ERPM_MODERATE     =  78000   # front getting close but not corner yet
ERPM_CORNER_FAST  =  66000   # wide / sweeping corner
ERPM_CORNER_TIGHT =  46000   # tight hairpin
ERPM_CRAWL        =  32000   # emergency near wall
...
GAP_SCALE         = 0.38   # servo throw during normal corners
GAP_SMOOTH        = 0.55   # faster gap response
CORNER_EXIT_LOOPS =  5     # faster exit blend back to corridor

# Tight U-turn boost: when front wall is very close, ramp up steering authority
UTURN_MM          =  520   # only tightest hairpins trigger U-turn boost
UTURN_SCALE_MAX   =  0.85  # strong authority when in a true U-turn
...
OBSTACLE_GAP_SCALE   = 0.50  # stronger steering during obstacle avoidance vs normal corner

# ZED blind-zone corridor centering.
# When LiDAR goes blind (black walls), use ZED left/right to gently keep the car
# centered. Very low gain + heavy smoothing = straight line, sparingly corrected.
ZED_CENTER_GAIN   = 0.04
ZED_CENTER_SMOOTH = 0.015
...
LIDAR_DIR   = os.path.join(_WS_ROOT, "v9LiDARData")
...
class CorridorNode(Node):
    def __init__(self):
        ...
        # Pure pursuit drive from pure_pursuit_node — used as primary steering in AUTONOMOUS mode
        self._pp_drive: AckermannDriveStamped | None = None
        self._pp_ts    = 0.0
        self.create_subscription(AckermannDriveStamped, "/pp_drive",
                                 self._pp_cb, 10)
    ...
    def _pp_cb(self, msg: AckermannDriveStamped):
        self._pp_drive = msg
        self._pp_ts    = time.time()

    # EMA smoothing for ZED readings — reduces frame-to-frame noise that causes zig-zag
    _ZED_FRONT_ALPHA = 0.45   # fast — detects black/orange front walls quickly
...
def main(args=None):
    ...
    for fname in os.listdir(LIDAR_DIR):
        if fname.startswith("v9LiDAR") and fname.endswith(".txt"):
            try:
                existing.append(int(fname[len("v9LiDAR"):-4]))
            except ValueError:
                pass
    next_num   = max(existing) + 1 if existing else 1
    lidar_path = os.path.join(LIDAR_DIR, f"v9LiDAR{next_num}.txt")
    lidar_file = open(lidar_path, "w")

    _DEG = 180.0 / math.pi
    _angles_deg = [round((44 + i - 384) * _SCAN_STEP * _DEG, 2) for i in range(_N_STEPS)]
    lidar_file.write("# v9 LiDAR Dataset — Hokuyo URG-04LX\n")
    ...
    node.get_logger().info("  corridor_node v9 — corridor + gap follow + mapping")
    node.get_logger().info(f"  Cruise: {ERPM_FULL} ERPM   Corner trigger: {CORNER_MM}mm")
    node.get_logger().info("  L1=kill  R1=cycle MANUAL→MAPPING→AUTONOMOUS")
    ...
    try:
        while rclpy.ok():
            ...
            if node.drive_mode == MODE_MANUAL:
                # Publish /scan while driving manually so slam_toolbox can build
                # the map and mapping_node can record the path. Without this,
                # SLAM gets no laser data in MANUAL and no waypoints are recorded.
            ...
            # ── Autonomous + Mapping modes (reactive nav) ──────
            # MODE_MAPPING (2) behaves exactly like MODE_AUTONOMOUS (0):
            # the car drives itself reactively while slam_toolbox builds the map.
            ...
            if GAP_FOLLOW_ENABLED and front_dist is not None and front_dist <= effective_corner_mm:
                ...
                mode_label  = "MAPPING" if node.drive_mode == MODE_MAPPING else "CORRIDOR"
                mode        = mode_label

                # BLIND zone: LiDAR can't see black walls.
                # Require 3 consecutive blind ticks (hysteresis) before locking in,
                # then use ZED left/right for gentle corridor centering.
                if steer_dbg == "BLIND":
                    blind_streak += 1
                    if blind_streak < 3:
                        steer_dbg = f"BLIND-WAIT({blind_streak})"
                    elif zed_fresh and node._zed_left is not None and node._zed_right is not None:
                        zl, zr = node._zed_left, node._zed_right
                        side_diff = (zr - zl) / max(zl + zr, 1.0)
                        # Deadband: ignore small asymmetries (noise), only correct if clearly off
                        if abs(side_diff) > 0.28:
                            zed_target = max(-1.0, min(1.0, side_diff * ZED_CENTER_GAIN))
                            steer = (1.0 - ZED_CENTER_SMOOTH) * steer + ZED_CENTER_SMOOTH * zed_target
                        steer_dbg = f"BLIND-ZED({side_diff:+.2f})"
                    # else: hold current heading — no ZED data available
                else:
                    blind_streak = 0
            ...
            # ── Pure pursuit override (AUTONOMOUS mode only) ───────
            # Two cases:
            #   BLIND zone (black walls, LiDAR has no data): PP always wins —
            #     reactive has nothing useful to offer, path follower knows the route.
            #   Normal zone (walls visible): PP leads in straights; reactive keeps
            #     control in corners/obstacles where it has real wall data.
            PP_STALE_S  = 0.40   # tolerate brief SLAM dropouts
            pp_fresh    = (time.time() - node._pp_ts) < PP_STALE_S
            is_blind    = blind_streak >= 3   # confirmed black-wall zone

            if node.drive_mode == MODE_AUTONOMOUS and pp_fresh and node._pp_drive is not None:
                pp_servo = max(SERVO_MIN, min(SERVO_MAX,
                               float(node._pp_drive.drive.steering_angle)))
                if is_blind:
                    # Black zone: follow the recorded path, ignore reactive noise
                    servo      = pp_servo
                    steer_dbg += " [PP-BLIND]"
                    mode       = "PP-BLIND"
                elif not in_corner and not obstacle:
                    # Visible straight: path follower leads
                    servo      = pp_servo
                    steer_dbg += " [PP]"
                    mode       = "PP"
            ...
            if lidar_confirmed_blind and target_erpm > ERPM_CORNER_FAST:
                target_erpm = ERPM_CORNER_FAST
                zone += "+BLIND-SLOW"
            ...
            if target_erpm > 0:
                ...
                prev_left_d = None; prev_right_d = None; prev_front_d = None
                boost_count = 0; was_stopped = True
                corner_exit_hold = 0; obstacle_hold = 0; erpm_ramp = 0.0
                speed_ctrl.reset()
