#!/usr/bin/env python3
"""
zed_obstacle_node.py — v8: ZED 2i depth obstacle avoidance + live display + recording.
...
Video with overlay saved to ~/Desktop/v8Video/videoN.mp4 on each run.
"""
...
OUTPUT_DIR  = os.path.expanduser("~/Desktop/v8Video")
...
class ZedGapNode(Node):
    ...
    def _start_camera(self):
        ...
        # COMP: QUALITY depth recovers more on low-texture black rubber than
        # PERFORMANCE, with depth stabilization to reduce frame-to-frame flicker.
    ...
    def _grab_loop(self):
        ...
        # COMP: keep only high-confidence depth. Stricter thresholds drop the
        # noisy, unreliable depth that black rubber produces — so a side ROI
        # reports "no data" (→ far) instead of a flickery false-close reading.
        try:
            runtime.confidence_threshold         = 50   # default 100; lower = stricter
            runtime.texture_confidence_threshold = 100  # reject low-texture guesses
        except Exception:
            pass
        img_mat   = sl.Mat()
        depth_mat = sl.Mat()
    ...
    # COMP: a side/front ROI must have at least this fraction of valid depth
    # pixels to be trusted. Below it, a handful of noise pixels could otherwise
    # fake a close wall — so we report DEPTH_MAX ("far/unknown") instead, which
    # the corridor node's trust-gate then rejects.
    MIN_VALID_FRAC = 0.08

    @classmethod
    def _nearest_m(cls, band: np.ndarray, c0: int, c1: int) -> float:
        region = band[:, c0:c1]
        valid  = region[(region >= DEPTH_MIN_M) & (region < DEPTH_MAX_M)]
        if region.size == 0 or valid.size < cls.MIN_VALID_FRAC * region.size:
            return DEPTH_MAX_M
