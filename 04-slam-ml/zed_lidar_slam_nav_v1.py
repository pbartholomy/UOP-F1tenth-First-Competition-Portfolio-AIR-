...
MAX_DUTY           = 0.11
AUTO_DRIVE_DUTY    = 0.068
MODERATE_DUTY_CAP  = 0.076
SLOW_DUTY_CAP      = 0.068
CRAWL_DUTY_CAP     = 0.062
MIN_EFFECTIVE_DUTY = 0.060
START_BOOST_DUTY   = 0.078
START_BOOST_LOOPS  = 14
REVERSE_DUTY       = 0.09
LOOP_HZ            = 50
DUTY_RAMP_STEP     = 0.007
CURVE_SLOW_STEER   = 0.58
CURVE_MIN_SCALE    = 0.58
...
# ── STEERING PID ─────────────────────────────────────────────
STEER_KP = 4.0    # proportional — step size scales with error magnitude
STEER_KI = 0.015  # integral     — eliminates persistent left/right drift
STEER_KD = 0.07   # derivative   — brakes before overshoot
...
BTN_X           = 0
BTN_CIRCLE      = 1
BTN_SQUARE      = 2   # NEW: reset SLAM map
BTN_TRIANGLE    = 3
BTN_TRIANGLE_ALT = 3
BTN_L1          = 4
BTN_L1_ALT      = 9
...
LIDAR_FULL_SPEED_MM = 1800
LIDAR_MODERATE_MM   = 1200
LIDAR_SLOW_MM       = 900
LIDAR_CRAWL_MM      = 600
LIDAR_ESTOP_MM      = 340
...
SIDE_PANIC_MM          = 500
SIDE_WARN_MM           = 420
FRONT_CORNER_WARN_MM   = 720
WALL_TARGET_MM         = 650

# ── ZED CAMERA ───────────────────────────────────────────────
CAM_ESTOP_M      = 0.32   # camera nearest-depth ESTOP threshold (m) — catches barriers LiDAR misses
ZED_ENABLED      = True
...
ZED_FWD_OFFSET_M     = 0.10   # ZED 2i is ~10 cm ahead of LiDAR center
ZED_PC_MAP_WEIGHT    = 0.45   # relative to SLAM_HIT for ZED contributions

# ── SLAM PARAMETERS (NEW) ────────────────────────────────────
SLAM_ENABLED     = True
SLAM_CELL_M      = 0.05          # 5 cm per cell
SLAM_MAP_M       = 24.0          # 24 m × 24 m
...
SLAM_TRUST_CELLS = 600           # min cells before map steer activates
...
MAP_STEER_WEIGHT  = 0.28                # max fraction of steering from map
MAP_CONF_CELLS    = 3000               # full confidence at this many cells

# Corner prediction
CORNER_DETECT_M    = 2.0
CORNER_FWD_THRESH  = 0.75              # fwd/CORNER_DETECT_M below this = possible corner
CORNER_SIDE_RATIO  = 1.5               # side/fwd above this = confirmed corner
CORNER_BRAKE       = 0.76              # speed scale when corner detected
CORNER_PRE_STEER   = 0.22             # steering bias toward open side on corner
...
BEHAVIOR_DEFAULT = BEHAVIOR_TRACK_FUSION   # use full fusion + SLAM by default
...
class ZEDDepthNavigator:
    def __init__(self):
        ...
        self._obstacle_pts = np.zeros((0, 2), dtype=np.float32)   # NEW: Nx2 robot-frame XY
    ...
    def _extract_obstacle_pts(self, pc_arr):
        ...
        Uses the middle vertical third of the image where obstacles at car-height appear.
    ...
    # ── corridor / floor scanners (unchanged from v2) ────────
...
class SlamMapper:
    ...
    def __init__(self):
        ...
        self._prev_pts = None   # Nx2 float32 in robot frame, previous scan
        ...
        # Cached outputs read by main loop
        self._c_lookahead = {a: LOOKAHEAD_M for a in LOOKAHEAD_ANGLES}
        self._c_corner    = (False, 1.0, 0.0)   # detected, brake_scale, pre_steer
        self._observed    = 0
    ...
    def _icp(self, src, dst):
        """
        2D point-to-point ICP. Returns (dx, dy, dtheta) from src→dst, or None if failed.
        """
    ...
    def _update_grid(self, pts_robot):
        ...
        # Transform scan endpoints to map frame
        ...
        # Free space: sample along each ray at 7 fractions (coarser, faster)
    ...
    # ── lookahead / corner (cached, updated in SLAM thread) ──
    ...
    def _compute_corner(self, lookahead):
        ...
        if fwd < CORNER_DETECT_M * CORNER_FWD_THRESH and max(left, right) > fwd * CORNER_SIDE_RATIO:
            if left > right:
                pre = CORNER_PRE_STEER    # left open → steer left (positive)
            else:
                pre = -CORNER_PRE_STEER   # right open → steer right (negative)
    ...
    def _run(self):
        while self._running:
            ...
            with self._lock:
                ...
                # Refresh cached outputs used by main loop
...
class SteerPID:
    """
    PD+I controller replacing the fixed-step ramp.
    D-term brakes before overshoot; I-term corrects persistent drift.
    """
    def __init__(self):
        self.reset()
...
def poll_imu_gyro_z(ser):
    ...
    try:
        ...
        if pkt is None or len(pkt) < 13 or pkt[0] != COMM_GET_IMU_DATA:
            return None
        # Gyro fields: x(4 B), y(4 B), z(4 B) — Z is yaw axis
...
def compute_gap_following(distances, clearances):
    if not distances:
        return 0.0
    return find_best_gap_direction(preprocess_gap_lidar(distances))
...
def compute_corridor_centering(clearances):
    ...
    return 0.0 if denom < 1.0 else max(-1.0, min(1.0, (left_d - right_d) / denom))
...
def compute_map_steer(lookahead, observed_cells):
    ...
    for angle_deg, dist in lookahead.items():
        ...
        if score > best_score:
            best_score = score
            # positive angle_deg = right direction = negative steer
...
def fuse_track_steer(lidar_gap, lidar_corridor, camera_data, clearances, front_dist, slam=None):
    ...
    lidar_base = 0.45 * lidar_gap + 0.40 * lidar_corridor + 0.15 * lidar_wall
    ...
    # Depth corridor is unreliable when barriers are closer than 0.80 m —
    # at that range the scan is looking at barrier surfaces, not the path ahead.
...
class LidarMapDisplay:
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("LiDAR + ZED + SLAM Nav")
...
class VideoRecorder:
    ...
    def write(self, frame):
        ...
        if self.writer is None:
            ...
            self.path = os.path.join(ZED_RECORD_DIR, f"zed_slam_{ts}.mp4")
            h, w     = frame.shape[:2]
...
def _find_vesc_port():
    """
    Return the first available VESC-like serial port, or None.
    Checks the stable by-id path first, then falls back to scanning /dev/ttyACM*.
    """
    import glob
...
def main():
    ...
    print(f"  SLAM: corner brake={CORNER_BRAKE:.2f} pre-steer={CORNER_PRE_STEER:.2f}")
    ...
    imu_heading         = 0.0   # integrated yaw in degrees (relative to startup)
    estop_entry_heading = 0.0   # heading snapshot when ESTOP fires
    imu_available       = False # set True on first successful poll
    loop_count          = 0

    # Post-ESTOP steer damping — caps steer magnitude for N loops after exit
    # so a panic response on loop 1 can't slam to full lock before car settles
    estop_exit_dampen   = 0
    ESTOP_EXIT_DAMP_LOOPS = 30
    ESTOP_EXIT_DAMP_CAP   = 0.75
    ...
    try:
        while True:
            ...
            # Steering PID — bypass instantly for panic responses
            _panic_kw = ("PANIC", "WARN", "FRONT_CLOSE")
            if any(kw in steer_source for kw in _panic_kw):
                current_steer = target_steer
                steer_pid.reset()
            else:
                current_steer = steer_pid.update(target_steer, current_steer, loop_period)

            # Post-ESTOP damping: cap steer magnitude for first N loops after exit
            # so a panic can't slam to full lock before the car has settled
            ...
            speed_cap, zone_label = lidar_speed_zone(front_dist, abs(current_steer), corner_brake)

            if estop_cooldown > 0:
                estop_cooldown -= 1

            # Camera nearest-depth ESTOP — catches barriers at heights/angles LiDAR misses
            ...
            # IMU polling — read VESC built-in gyro every IMU_POLL_EVERY loops
            loop_count += 1
